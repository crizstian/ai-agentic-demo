import re

from flask import Blueprint, jsonify, request

from ..db import get_db

transfers_bp = Blueprint("transfers", __name__)

_ACCOUNT_ID_RE = re.compile(r'^[A-Za-z0-9\-]{1,64}$')
_MAX_TRANSFER_AMOUNT = 1_000_000


@transfers_bp.route("/", methods=["POST"])
def create_transfer():
    body = request.get_json(silent=True) or request.form
    from_account = body.get("fromAccount")
    to_account = body.get("toAccount")
    amount = body.get("amount")
    memo = body.get("memo", "")

    if not from_account or not to_account or amount is None:
        return jsonify({"error": "Missing required fields"}), 400

    if not _ACCOUNT_ID_RE.match(str(from_account)) or not _ACCOUNT_ID_RE.match(str(to_account)):
        return jsonify({"error": "Invalid account ID format"}), 400

    try:
        amount = float(amount)
    except (TypeError, ValueError):
        return jsonify({"error": "Amount must be a number"}), 400

    if amount <= 0 or amount > _MAX_TRANSFER_AMOUNT:
        return jsonify({"error": "Amount must be greater than zero and at most 1,000,000"}), 400

    if from_account == to_account:
        return jsonify({"error": "Source and destination accounts must differ"}), 400

    db = get_db()
    cursor = db.execute(
        "INSERT INTO transactions (from_account, to_account, amount, memo, status) VALUES (?, ?, ?, ?, ?)",
        [from_account, to_account, amount, str(memo)[:500], "completed"],
    )
    db.commit()
    return jsonify(
        {
            "success": True,
            "message": "Transfer completed successfully",
            "transferId": cursor.lastrowid,
            "amount": amount,
        }
    )


@transfers_bp.route("/", methods=["GET"])
def list_transfers():
    db = get_db()
    rows = db.execute(
        "SELECT * FROM transactions ORDER BY created_at DESC LIMIT 20"
    ).fetchall()
    return jsonify([dict(r) for r in rows])
